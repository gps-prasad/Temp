...
function CustomTable() {
  ...
  return (
    <div style={{height:'100%'}} className='w-[100%] overflow-auto scrollbar-hide p-5 box-border bg-white max-h-[100%]'>
        <div>
      ...
      <div className="grid grid-cols-[1fr,1fr,2fr,3fr,2fr,2fr,2fr] gap-0.5 px-5 w-full font-bold my-1">
        {/* Heading */}
        <div className='text-start py-2 space-x-2 flex text-cyan-600 hover:text-cyan-600 cursor-pointer'><input className='text-cyan-200' type='checkbox' onChange={selectAll}/><FiSearch size={25} onClick={()=>setShowFilters((prev)=>!prev)}/></div>
      ...
      {/* Filters */}
      {showFilters && <div className="grid grid-cols-[1fr,1fr,2fr,3fr,2fr,2fr,2fr] gap-0.5 px-5 w-full py-1 bg-gray-200">
        <div className='text-start py-2 space-x-2 flex text-cyan-600 hover:text-cyan-600 cursor-pointer'></div>
        ...
        {filteredUsers && filteredUsers.slice((currentPage-1)*10,currentPage*10).map((user,id)=>{
          return (
            <div className={`grid grid-cols-[1fr,1fr,2fr,3fr,2fr,2fr,2fr] gap-0.5 py-2 px-5 hover:bg-cyan-200 hover:border-b hover:text-cyan-600 hover:border-b-cyan-600 ${id%2===1?"bg-gray-100":"bg-white"}`} key={user.userId}>
      ...
      <div className='mb-2 mt-4 w-[100%] box-border'>
        {totalPages===1?<></>:<Pagination showControls variant="bordered" classNames={{cursor:'bg-cyan-600',wrapper:'mx-auto'}} initialPage={1} total={totalPages} onChange={setCurrentPage}/>}
      </div>
    </div>
  )
}
